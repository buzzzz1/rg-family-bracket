...
local.run("state.results.men.r0[0] = 1");
assert.equal(local.run("playerTournamentStatus('men', 0).label"), 'Eliminated');
assert(local.run("playerTournamentStatus('men', 0).detail").includes('Round of 128'));
assert(local.run("state.playerModal = {ev:'men',slot:0,pair:null}; playerModalHTML()").includes('× Eliminated'));
local.run("state.config.locked = true; state.entries.chloe = {name:'Chloe', men: emptyPicks(), women: emptyPicks()}; state.entries.chloe.men.r0[0] = 0");
const poolCard = local.run("state.results.men.r0[0] = null; state.playerModal = {ev:'men',slot:0,pair:null}; playerModalHTML()");
assert(poolCard.includes('Picked in 1 of 1 submitted bracket'));
assert(poolCard.includes('Chloe'));
assert(poolCard.indexOf('In the family brackets') > poolCard.indexOf('Still in tournament'));
assert(poolCard.indexOf('In the family brackets') < poolCard.indexOf('Country'));
local.run("state.config.locked = false");
local.run("state.results.men.r0[0] = null");
